...
public final class ConfigurationCollector {
        ...
        for (final Field f : Configuration.class.getFields()) {
            if (Modifier.isStatic(f.getModifiers()) && Modifier.isFinal(f.getModifiers())) {
                ...
                try {
                    ...
                    Log.w(CrashReporter.LOG_TAG, "Error while inspecting device configuration: ", e);
    ...
    public static String toString(Configuration conf) {
        ...
        for (final Field f : conf.getClass().getFields()) {
            try {
                ...
                Log.e(CrashReporter.LOG_TAG, "Error while inspecting device configuration: ", e);
            } catch (IllegalAccessException e) {
                Log.e(CrashReporter.LOG_TAG, "Error while inspecting device configuration: ", e);
            }
        }
    ...
    public static String collectConfiguration(Context context) {
        try {
            ...
            Log.w(CrashReporter.LOG_TAG, "Couldn't retrieve CrashConfiguration for : " + context.getPackageName(), e);
