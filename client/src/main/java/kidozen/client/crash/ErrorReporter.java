...
public class ErrorReporter implements Thread.UncaughtExceptionHandler {
     ...
     * @param prefs
     *            SharedPreferences used by CrashReporter.
     * @param enabled
        ...
        if (Compatibility.getAPILevel() >= 14) { // ActivityLifecycleCallback
            ...
            ApplicationHelper.registerActivityLifecycleCallbacks(context, new ActivityLifecycleCallbacksCompat() {
                ...
                public void onActivityCreated(Activity activity, Bundle savedInstanceState) {
                    if (CrashReporter.DEV_LOGGING)
                        CrashReporter.log.d(CrashReporter.LOG_TAG, "onActivityCreated " + activity.getClass());
                    /*
                ...
                public void onActivityStarted(Activity activity) {
                    if (CrashReporter.DEV_LOGGING)
                        CrashReporter.log.d(CrashReporter.LOG_TAG, "onActivityStarted " + activity.getClass());
                }

                @Override
                public void onActivityResumed(Activity activity) {
                    if (CrashReporter.DEV_LOGGING)
                        CrashReporter.log.d(CrashReporter.LOG_TAG, "onActivityResumed " + activity.getClass());
                }

                @Override
                public void onActivityPaused(Activity activity) {
                    if (CrashReporter.DEV_LOGGING)
                        CrashReporter.log.d(CrashReporter.LOG_TAG, "onActivityPaused " + activity.getClass());
                }

                @Override
                public void onActivityStopped(Activity activity) {
                    if (CrashReporter.DEV_LOGGING)
                        CrashReporter.log.d(CrashReporter.LOG_TAG, "onActivityStopped " + activity.getClass());
                }

                @Override
                public void onActivitySaveInstanceState(Activity activity, Bundle outState) {
                    if (CrashReporter.DEV_LOGGING)
                        CrashReporter.log.i(CrashReporter.LOG_TAG, "onActivitySaveInstanceState " + activity.getClass());
                }

                @Override
                public void onActivityDestroyed(Activity activity) {
                    if (CrashReporter.DEV_LOGGING)
                        CrashReporter.log.i(CrashReporter.LOG_TAG, "onActivityDestroyed " + activity.getClass());
                }
            });
        }
    ...
    public void uncaughtException(Thread t, Throwable e) {
        try {
            ...
            Log.e(CrashReporter.LOG_TAG,
                    "ACRA caught a " + e.getClass().getSimpleName() + " exception for " + mContext.getPackageName()
                            + ". Building report.");

            // Generate and send crash report
            handleException(e, CrashReporter.getConfig().mode(), false, true);
        } catch (Throwable fatality) {
            // ACRA failed. Prevent any recursive call to
            // CrashReporter.uncaughtException(), let the native reporter do its job.
            if (mDfltExceptionHandler != null) {
                mDfltExceptionHandler.uncaughtException(t, e);
            }
        }
    ...
    private void endApplication() {
        if (CrashReporter.getConfig().mode() == ReportingInteractionMode.SILENT
                || (CrashReporter.getConfig().mode() == ReportingInteractionMode.TOAST && CrashReporter.getConfig()
            ...
            Log.e(CrashReporter.LOG_TAG, mContext.getPackageName() + " fatal error : " + unhandledThrowable.getMessage(),
                    unhandledThrowable);
            ...
            if (lastActivityCreated != null) {
                Log.i(CrashReporter.LOG_TAG, "Finishing the last Activity prior to killing the Process");
                lastActivityCreated.finish();
                Log.i(CrashReporter.LOG_TAG, "Finished " + lastActivityCreated.getClass());
    ...
    public void checkReportsOnApplicationStart() {
        ...
        final long lastVersionNr = prefs.getInt(CrashReporter.PREF_LAST_VERSION_NR, 0);
        ...
        if (newVersion) {
            if (CrashReporter.getConfig().deleteOldUnsentReportsOnApplicationStart()) {
                deletePendingReports();
            }
            final SharedPreferences.Editor prefsEditor = prefs.edit();
            prefsEditor.putInt(CrashReporter.PREF_LAST_VERSION_NR, packageInfo.versionCode);
            prefsEditor.commit();
        }

        if ((CrashReporter.getConfig().mode() == ReportingInteractionMode.NOTIFICATION || CrashReporter.getConfig().mode() == ReportingInteractionMode.DIALOG)
                && CrashReporter.getConfig().deleteUnapprovedReportsOnApplicationStart()) {
        ...
        if (filesList != null && filesList.length > 0) {
            ...
            ReportingInteractionMode reportingInteractionMode = CrashReporter.getConfig().mode();
            ...
            if (reportingInteractionMode == ReportingInteractionMode.SILENT
                ...
                if (reportingInteractionMode == ReportingInteractionMode.TOAST && !onlySilentOrApprovedReports) {
                    ...
                    // non-silent reports.
                    ToastSender.sendToast(mContext, CrashReporter.getConfig().resToastText(), Toast.LENGTH_LONG);
                }
                */
                Log.v(CrashReporter.LOG_TAG, "About to start ReportSenderWorker from #checkReportOnApplicationStart");
                startSendingReports(false, false);
            } else if (CrashReporter.getConfig().mode() == ReportingInteractionMode.NOTIFICATION) {
            ...
            } else if (CrashReporter.getConfig().mode() == ReportingInteractionMode.DIALOG) {
    ...
    public void handleException(Throwable e, boolean endApplication) {
        handleException(e, CrashReporter.getConfig().mode(), false, endApplication);
    }
    ...
    public void handleException(Throwable e) {
        handleException(e, CrashReporter.getConfig().mode(), false, false);
    }
    ...
    private void handleException(Throwable e, ReportingInteractionMode reportingInteractionMode,
        ...
        if (reportingInteractionMode == null) {
            ...
            // CrashReporter.initACRA()
            reportingInteractionMode = CrashReporter.getConfig().mode();
        } else {
            ...
            if (reportingInteractionMode == ReportingInteractionMode.SILENT
                    && CrashReporter.getConfig().mode() != ReportingInteractionMode.SILENT) {
        ...
        final boolean shouldDisplayToast = reportingInteractionMode == ReportingInteractionMode.TOAST
                || (CrashReporter.getConfig().resToastText() != 0 && (reportingInteractionMode == ReportingInteractionMode.NOTIFICATION || reportingInteractionMode == ReportingInteractionMode.DIALOG));
/*
...
*/
        ...
        if (reportingInteractionMode == ReportingInteractionMode.SILENT
                || reportingInteractionMode == ReportingInteractionMode.TOAST
                || prefs.getBoolean(CrashReporter.PREF_ALWAYS_ACCEPT, false)) {

            // Approve and then send reports now
            Log.d(CrashReporter.LOG_TAG, "About to start ReportSenderWorker from #handleException");
            sender = startSendingReports(sendOnlySilentReports, true);
        } else if (reportingInteractionMode == ReportingInteractionMode.NOTIFICATION) {
            Log.d(CrashReporter.LOG_TAG, "Notification will be created on application start.");
        }
        ...
        new Thread() {
            ...
            public void run() {
                ...
                if (showDirectDialog) {
                    ...
                    // right
                    // after its death.
                    Log.d(CrashReporter.LOG_TAG, "About to create DIALOG from #handleException");
        ...
        if (filesList != null) {
            for (int iFile = 0; iFile < filesList.length - nbOfLatestToKeep; iFile++) {
                ...
                if ((isReportApproved && deleteApprovedReports) || (!isReportApproved && deleteNonApprovedReports)) {
                    ...
                    CrashReporter.log.d(CrashReporter.LOG_TAG, "Deleting file " + fileName);
                    if (!fileToDelete.delete()) {
                        Log.e(CrashReporter.LOG_TAG, "Could not delete report : " + fileToDelete);
                    }
                }
            }
        }
